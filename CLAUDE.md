...
`poll()` refuses to measure when `_tick_start` belongs to a tick no live task owns. `stop()`, the `_monitor_loop` error handler, and the loop's exit all clear `_tick_start`, and `poll()` additionally bails when `_task` is done. Without this a cancelled or backing-off monitor makes `poll()` report a stall that grows with wall-clock time — in strict mode, a 503 for a request that did nothing wrong. **Never gate this on a maximum tick age**: a real 5-second stall is exactly the case the library exists to catch. `BlockingDetector.stop()` in `pytest_plugin.py` has the same problem and the same shape of fix — it clears `_running` and **drains** the monitor task instead of cancelling it, so the expired sleep resumes and records before teardown.
...
## Known Gaps (accurate as of 2026-09-06)
