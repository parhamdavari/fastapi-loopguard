...
@pytest.mark.asyncio
async def test_cumulative_blocking_detection():
    ...
    def on_blocking(lag_ms, path, method):
    ...
    try:
        ...
        # Note: The first detection might clear the history, so subsequent ones might be smaller or larger depending on timing.
        ...
        assert len(cumulative_detections) > 0, f"Detected events {detected_events} did not exceed cumulative threshold 100ms"

    finally:
        await monitor.stop()

@pytest.mark.asyncio
async def test_cumulative_blocking_disabled_by_default():
    ...
    detected_events = []
    monitor = SentinelMonitor(config, on_blocking=lambda x, y, z: detected_events.append(x))
    await monitor.start_with_background_calibration()
    await asyncio.sleep(0.2)
    ...
    try:
        ...
        await asyncio.sleep(0.2)
        assert len(detected_events) == 0, "Should NOT detect blocking when cumulative detection is disabled"
    finally:
        await monitor.stop()
